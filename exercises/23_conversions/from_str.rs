...
impl FromStr for Person {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        let name_str = data.next();
        let age_str = data.next();
        let anything_else = data.next();

        if anything_else.is_some() {
        ...
        let name = name_str.expect("already checked");
        if name.is_empty() {
            return Err(Self::Err::NoName)
        }

        if age_str.is_none() {
            let parse_empty_error = "".parse::<u8>().unwrap_err();
            return Err(Self::Err::ParseInt(parse_empty_error))
        }
        let age = age_str.expect("Already checked").parse::<u8>();
        ...
        Ok(Person {
                name: name.to_string(),
                age: age.expect("already checked")
        })
